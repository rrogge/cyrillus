...
Crater center
```{r}
crater <- list(center=list(l = 24.07, b = -13.29), radius=50)
```
...
profile.135_315 <- read.profile("input/profile_135-315.txt", direction="135-315")
```
...
r <- distance(profile.135_315$l, profile.135_315$b, crater$center$l, crater$center$b)
profile.135_315$r <- sign(profile.135_315$a - profile.000_180$a[which.min(r)]) * r
```

Combine profile data
```{r}
data <- rbind(rbind(profile.000_180, profile.090_270), profile.135_315) %>%
